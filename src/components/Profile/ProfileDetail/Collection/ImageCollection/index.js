...
const ImageCollection = ({id, onNestedScroll}) => {
    ...
    const fetchData = () => {
        AsyncStorage.getItem('tokenLogin').then(token => {
            axios.get(`${SERVERIP}/getImageVideoOfProfile?accountId=${id}&type=0&pageIndex=${pageIndex}&pageSize=18`, {
                headers: {
            ...
            >
                {
                    data.map((item, index) => {
                        return (
                            <View key={index}>
                                <ImageBlock id={id} item={item} onIndex={index} onAllData={handleAllData}/>
                            </View>
                        )
                    })
                }
